...
from training.train_nisq import (
    run_nisq_training,
    evaluate_clean as nisq_eval_clean,
    evaluate_asr as nisq_eval_asr,
)
...
def write_results_csv(results: List[Dict[str, float]], output_dir: Path) -> Path:
...
def train_method(
    ...
    if method == "nisq":
        ...
        print(
            "{} training complete. clean_acc={:.4f} asr={:.4f}".format(
                method,
                result.get("checkpoint_selection", {}).get("clean_acc", 0.0),
...
def run_comparison(
    ...
    print(
        "Dataset: {}  poison_rate={}  target_label={}  epsilon={}".format(
            config.data.dataset,
            config.train.poison_rate,
            config.train.target_label,
            config.train.epsilon,
        )
    )
    print()
    ...
    if skip_train:
        raise NotImplementedError(
            "skip_train=True is declared but checkpoint-loading logic is not implemented."
        )
    ...
    for method in METHODS:
        if method == "nisq":
            ...
            if generator is None:
                raise RuntimeError("NISQ evaluation requires a generator.")

            result = evaluate_nisq(victims[method], generator, config, device)
        else:
        ...
        print(
            "  {:<12s}  clean_acc={:.4f}  asr={:.4f}".format(
                method,
                result.get("clean_acc", 0.0),
                result.get("asr", 0.0),
            )
