...
from models.factory import build_classical_inr_generator, build_classifier
...
from training.optim import build_classical_inr_optimizer
...
def default_output_dir(config) -> Path:
    ...
    return Path(config.train.save_dir) / "classical_inr_backdoor_{}".format(stamp)


def parse_args():
    parser = argparse.ArgumentParser(
        description="Classical INR backdoor attack with the same experiment flow as nisq_run."
    )
...
def _load_trained_models(config, checkpoint_path: str | Path, device):
    ...
    generator = build_classical_inr_generator(config).to(device)
...
def main() -> None:
    ...
    result = run_nisq_training(
        ...
        generator_builder=build_classical_inr_generator,
        generator_optimizer_builder=build_classical_inr_optimizer,
        experiment_name="classical_inr_backdoor",
        log_label="classical inr",
    )
    ...
    post_training_artifacts = {
        ...
        "visualization_metrics_json": str(
            vis_dir / "classical_inr_poisoned_samples_metrics.json"
        ),
    ...
    print("=" * 60)
    print("Classical INR backdoor training complete.")
